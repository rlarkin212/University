<%@ Page Title="Timetable" Language="C#" MasterPageFile="Student.Master" AutoEventWireup="true" CodeBehind="StudentTimetable.aspx.cs" Inherits="CIT12.Student.StudentTimetable" %>
...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <asp:Panel ID="pnlTimeTable" runat="server">
        ...
        <div class="row">
            <asp:GridView ID="gvStudentTimetable" runat="server" DataSourceID="dsStudentTimetable" AutoGenerateColumns="False" DataKeyNames="Id" CssClass="table table-responsive table-striped table-condensed table-hover" EmptyDataText="No Timetable Is Found For This User, Check You Have Been Enrolled Correctly" GridLines="None" CellSpacing="-1" OnRowDataBound="gvStudentTimetable_RowDataBound">
                <Columns>
                    ...
                    <asp:BoundField DataField="Day" HeaderText="Day" SortExpression="Day" ItemStyle-CssClass="font-weight-bold"></asp:BoundField>
            ...
            <hr />
            <asp:Button ID="btnPrintGv" runat="server" Text="Print" ToolTip="Print Timetable" CssClass="btn btn-primary btn-lg" OnClientClick="printGV()" />
        </div>
        ...
        <asp:SqlDataSource runat="server" ID="dsStudentTimetable" ConnectionString='<%$ ConnectionStrings:ApplicationConnection %>' SelectCommand="SELECT DISTINCT tbl_timeTable.Id, tbl_days.Day, tbl_modules.Title, tbl_classType.Type, tbl_timeTable.Time, tbl_timeTable.Location, tbl_studentModule.StudentId, tbl_timeTable.DayId FROM tbl_timeTable INNER JOIN tbl_days ON tbl_timeTable.DayId = tbl_days.Id INNER JOIN tbl_modules ON tbl_timeTable.ModuleId = tbl_modules.Id INNER JOIN tbl_classType ON tbl_timeTable.ClassTypeId = tbl_classType.Id INNER JOIN tbl_studentModule ON tbl_timeTable.ModuleId = tbl_studentModule.ModuleId WHERE (tbl_studentModule.StudentId = @loggedUserId) ORDER BY tbl_timeTable.DayId ASC">
            <SelectParameters>
