...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <asp:Panel ID="pnlModuleList" runat="server">
        ...
        <asp:SqlDataSource runat="server" ID="dsModuleList" ConnectionString='<%$ ConnectionStrings:ApplicationConnection %>' SelectCommand="SELECT tbl_modules.Id, tbl_modules.Title, tbl_modules.Code, tbl_modules.Level, tbl_modules.CatPoints, tbl_semester.Semester, tbl_course.Name FROM tbl_modules INNER JOIN tbl_semester ON tbl_modules.SemesterId = tbl_semester.Id INNER JOIN tbl_course ON tbl_modules.CourseId = tbl_course.Id ORDER BY tbl_modules.Level ASC" FilterExpression="Code LIKE '{0}%'">
    ...
    <asp:Panel ID="pnlClassList" runat="server" Visible="false">
        ...
        <asp:SqlDataSource runat="server" ID="dsAcademicModuleClassList" ConnectionString='<%$ ConnectionStrings:ApplicationConnection %>' SelectCommand="SELECT tbl_user.Id, tbl_user.Number, CONCAT(tbl_user.Forename, ' ', tbl_user.Surname)  AS FullName, tbl_user.EmailAddress, tbl_years.Year, tbl_years.Id AS YearId FROM tbl_user INNER JOIN tbl_studentModule ON tbl_user.Id = tbl_studentModule.StudentId INNER JOIN tbl_years ON tbl_studentModule.YearId = tbl_years.Id WHERE (tbl_studentModule.ModuleId = @moduleId) ORDER BY FullName">
            <SelectParameters>
